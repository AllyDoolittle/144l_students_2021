...
A1: The reads are long enough to get the primers on the end. Will use the MergePairs function by adding trimOverhang=T.
...
A2: Where the quality of the profile drops off (last ten nucleotides, at 240 bp). We do this to avoid some of the less controlled errors that arrive at the end of the run.
...
A3:  More poor in quality than the forward reads, which is common for Illumina sequencing. Yes because DADA2 incorporates quality information in an error model that we will run later.

Q4: Where will we trim our reverse reads and why?
A4: At 150 bp because that's where the quality drops off and trimming helps improve the algorithm sensitivity.
...
A5: The sequencer doesn't know what nucleotide to put into it so it puts "N". No. We remove these ambiguous "N"s by running "maxN = 0". 
...
A7: The error rates are lower when the quality score of the reads are higher, and vice versa. 
...
A9: The number of base overlap matches in the forward and reverse regions.
